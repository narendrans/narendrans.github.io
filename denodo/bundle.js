...
function remoteSource(parent, cfg, callback) {
  ...
  var xhrUid = ++xhrCnt;
  var type = 'name';
 var url = 'http://ec2-54-236-91-8.compute-1.amazonaws.com:9090/server/admin/list_databases/views/p_list_databases?$format=json';
 console.log(parent);
  // determine which column we're on based on previous selection
if(parent){
  if(parent.type=='database')
    url='http://ec2-54-236-91-8.compute-1.amazonaws.com:9090/server/admin/list_databases/views/p_list_views?DATABASE='+parent.label+'&$format=json';
  else if(parent.type='view'){
    if(!parent.DATABASE)
      return cfg.emitter.emit('create-column', createSimpleColumn(parent));
    else
    url = 'http://ec2-54-236-91-8.compute-1.amazonaws.com:9090/server/admin/list_databases/views/p_viewinfo?DBVIEW='+parent.DATABASE+'.'+parent.label+'&$format=json';
  }
...
function createItemContent(cfg, item) {
  ...
  if (item.type === 'database') {
    prependClasses.push('fa-database');
  } else if (item.type === 'view') {
    prependClasses.push('fa-table');
  } else {
    prependClasses.push('fa-info');
  }
...
function createSimpleColumn(item) {
  ...
  var i = _.el('i');
  var fieldName = _.el('div.meta');
  var fieldType = _.el('strong');
  var population = _.el('div.meta');
  var populationLabel  = _.el('strong');

  _.addClass(i, ['fa', 'fa-info-circle']);
  _.append(fieldType, _.text('Data Type: '));
  _.append(fieldName, [fieldType, _.text(item.fieldtype)]);
  _.append(populationLabel , _.text('Is Primary Key?: '));
  _.append(population, [
    populationLabel,
    _.text(item.isPrimaryKey)
  ]);
  _.append(row, [i, filename, fieldName, population]);
